...
- **Simple Trailing Guard (NEW)**: After a trade covers fees, reaches ≥0.8% peak PnL, and builds at least `initial_balance × 3%` of profit, giving back 35% of that risk anchor forces a lock-in; keep it on (default) or toggle with `simple_trailing_guard_enabled`, tune polling via `guard_interval_minutes`, and adjust the fee gate via `simple_trailing_fee_pct`.
