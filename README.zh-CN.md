...
- **简易回撤守护（NEW）**：当浮盈覆盖手续费并达到 ≥0.8% 且至少赚到 `initial_balance × 3%` 的风险锚点时，只要回吐该锚点的 35% 就会强制锁盈；默认开启，可用 `simple_trailing_guard_enabled` 控制开关、`guard_interval_minutes` 调整频率，`simple_trailing_fee_pct` 自定义回本阈值。
